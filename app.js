...
class UI{
    displayProducts(products){
        ...
        products.forEach(product =>{
            ...
            const p = document.querySelector(".product-pricing");
            p.append(productDiv)
        })
    }
...
addCartItem(cartItem) {
    ...
    cartContent.addEventListener("click",(event)=>{
        if(event.target.classList.contains("cart-product-remove")){
            ...
            if(item.amount>1){
                ...
                Storage.saveCart(Cart)
                this.setCartValues(Cart)
                event.target.prevElementSibling.innerHTML = item.amount
            }
            else{
                this.removeItem(id)
                let div = event.target.parentElement.parentElement.parentElement.parentElement
   ...
   addAmount(){
    ...
    redudeAmount(){
        const reduceBtn = document.querySelectorAll(".reduce-amaount")
